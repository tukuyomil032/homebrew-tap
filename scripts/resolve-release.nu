...
def main [repo: string, channel: string, asset_template: string] {
  ...
  let release = if $channel == 'stable' {
    ...
    try {
      open $body_file --raw | from json
    } catch { error make {msg: 'failed to parse release JSON'} }
    ...
    let published = (
      $releases
      | where draft != true
      | sort-by {|release| ($release | get -o published_at | default ($release | get -o created_at | default '')) }
      | reverse
    )
